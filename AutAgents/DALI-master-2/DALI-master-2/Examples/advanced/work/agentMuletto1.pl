...
eve(richiesta_deposito_prodotto(var_Prodotto,var_Qta)):-not(deposito(var__,var__)),not(prelievo(var__,var__)),format('Richiesta deposito ~w',var_Prodotto),nl,assertz(deposito(var_Prodotto,var_Qta)),a(message(agentMagazzino,send_message(richiesta_deposito_prodotto(var_Me,var_Prodotto),var_Me))).

eve(richiesta_deposito_prodotto(var_Prodotto,var_Qta)):-deposito(var_X,var_Y),write('Sono impegnato in un deposito').

eve(richiesta_deposito_prodotto(var_Prodotto,var_Qta)):-prelievo(var_X,var_Y),write('Sono impegnato in un prelievo').

eve(risposta_deposito_prodotto(var_Prodotto,var_Settore)):-a(vado_al_settore(var_Settore)),deposito(var_Prodotto,var_Qta),a(deposito(var_Prodotto,var_Qta,var_Settore)),a(torno_all_hub),retractall(deposito(var__,var__)).

a(deposito(var_Prodotto,var_Qta,var_Settore)):-sleep(3),a(message(agentMagazzino,send_message(prodotto_depositato(var_Prodotto,var_Qta,var_Settore),var_Me))).

eve(richiesta_prelievo_prodotto(var_Prodotto,var_QtaRichiesta)):-not(deposito(var__,var__)),not(prelievo(var__,var__)),format('Sto prelevando il prodotto ~w',var_Prodotto),nl,assertz(prelievo(var_Prodotto,var_QtaRichiesta)),a(message(agentMagazzino,send_message(richiesta_prelievo_prodotto(var_Me,var_Prodotto,var_QtaRichiesta),var_Me))).

eve(richiesta_prelievo_prodotto(var_Prodotto,var_Qta)):-deposito(var__,var__),write('Sono impegnato in un deposito').

eve(richiesta_prelievo_prodotto(var_Prodotto,var_Qta)):-prelievo(var__,var__),write('Sono impegnato in un prelievo').

eve(risposta_prelievo_prodotto(var_Prodotto,var_Settore)):-write('Entro'),var_X=null,var_Settore\==null,prelievo(var_Prodotto,var_QtaRichiesta),a(vado_al_settore(var_Settore)),a(prelevo(var_Prodotto,var_QtaRichiesta)),a(torno_all_hub).

eve(risposta_prelievo_prodotto(var_Prodotto,var_Settore)):-write('Entro2'),var_X=null,var_Settore==var_X,prodotto(var_Prodotto,var_QtaRichiesta),write([118,97,114,95,73,108,32,112,114,111,100,111,116,116,111,32,110,111,110,32,112,117,111,32,101,115,115,101,114,101,32,112,114,101,108,101,118,97,116,111,32,112,111,105,99,104,101,32,110,111,110,32,99,105,32,115,111,110,111,32,97,98,98,97,115,116,97,110,122,97,32,117,110,105,116,97]),a(torno_all_hub).

a(prelevo(var_Prodotto,var_Qta)):-format([118,97,114,95,80,114,101,108,101,118,111,32,105,108,32,112,114,111,100,111,116,116,111,32,126,119],var_Prodotto),sleep(3),a(message(agentMagazzino,send_message(prodotto_prelevato(var_Prodotto,var_Qta),var_Me))).

a(vado_al_settore(var_Settore)):-format([118,97,114,95,86,97,100,111,32,97,108,32,115,101,116,116,111,114,101,32,126,119],var_Settore),sleep(3).

a(operazione_completata):-retractall(deposito(var__,var__)),retractall(prelievo(var__,var__)).

a(torno_all_hub):-sleep(3),a(operazione_completata).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_370869,var_Ontology,_370873),_370863),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_370907)),a(message(var_Ag,propose(var_A,[_370907],var_AgI))),retractall(ext_agent(var_Ag,_370945,var_Ontology,_370949)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_370743,var_Ontology,_370747),_370737),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_370813,var_Ontology,_370817)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_370631,var_Ontology,_370635),_370625),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_370687,var_Ontology,_370691)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_370195),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_370229),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_370011).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_369859),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_369807),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_369683,_369685)),clause(agente(_369705,_369707,_369709,var_S),_369701),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_369477,_369479,_369481,var_S),_369473),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_369577,[]),repeat,read(_369577,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_369577).
